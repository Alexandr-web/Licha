...
class Chart {
	...
	_drawWindowInfoBlock() {
		...
		let { top: { x: xGroupName, }, bottom: { x: xGroupValue, }, } = containPositions;
		let xInfoBlock = x + windowPadding.fromCap;

		const { top: { y: yGroupName, }, bottom: { y: yGroupValue, }, } = containPositions;
		const groupLine = containPositions.line;
		const yInfoBlock = y - windowBlock.height / 2;
		const { width: currentCanvasWidth, } = this._getCanvasSizes();

		// Если блок выйдет за пределы канваса по оси x
		if (x + windowBlockWidth > currentCanvasWidth) {
			xInfoBlock = (x - windowBlockWidth) - windowPadding.fromCap;
			xGroupName = xInfoBlock - radius + windowPadding.fromCap;
			xGroupValue = xInfoBlock - radius + windowPadding.fromCap;

			const { start, to, } = groupLine;

			start.x = xInfoBlock - radius + windowPadding.fromCap + windowBlock.width - windowPadding.horizontal;
			to.x = xInfoBlock - radius + windowPadding.fromCap + windowBlock.width - windowPadding.horizontal;

			start.y = y - windowBlock.height / 2 + windowPadding.vertical;
			to.y = y - windowBlock.height / 2 + windowBlock.height - windowPadding.vertical;
		}

		// Рисуем блок окна
		windowBlock.drawWindow(xInfoBlock, yInfoBlock);
		// Рисуем название группы
		windowBlock.drawContains(windowContains.top, xGroupName, yGroupName);
		// Рисуем значение
		windowBlock.drawContains(windowContains.bottom, xGroupValue, yGroupValue);
		// Рисуем линию группы
		windowBlock.drawGroupLine(groupLine);
	}
